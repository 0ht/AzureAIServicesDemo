"""Azure Content Understanding クライアント ユーティリティ

このモジュールは Azure AI Services の Content Understanding (プレビュー/想定) API を Python から
簡潔に呼び出すためのラッパー実装です。主な機能は以下の通り:

1. アナライザー (Analyzer) / クラシファイア (Classifier) の作成・削除・一覧取得
2. 単一または複数ファイル (Pro モード) の解析ジョブ送信と結果ポーリング
3. 事前構築 (prebuilt) ドキュメント アナライザー利用 (`prebuilt-documentAnalyzer`)
4. トレーニング用データ / 参照ドキュメント (knowledge sources) の BLOB 生成支援
5. 分類(Classify) API 呼び出し
6. 解析ジョブ中に生成される OCR 結果(JSON) や sources.jsonl の BLOB へのアップロード

設計上のポイント:
 - API 呼び出しは requests によるシンプルな同期 HTTP (一部 BLOB 送信は async) を採用
 - Azure Storage へのアップロードは official SDK (同期/非同期混在) を利用
 - ポーリングはシンプルなループ (指数バックオフ無し) -> 必要に応じて改善余地あり
 - サポート拡張を容易にするため URL 生成は専用メソッドに分離

注意/改善余地:
 - ポーリングのタイムアウト/インターバルはハードコード (定数化済みだが外部設定も検討可)
 - エラー詳細は一部そのまま raise。用途に応じより粒度の細かい例外クラス導入余地あり
 - 大量ファイル (フォルダ一括) 解析時の base64 化はメモリ消費に注意 (ストリーミング最適化可能)
 - 認証は Subscription Key か Token Provider (Bearer) のいずれか。DefaultAzureCredential は BLOB SAS 生成目的のみ使用

各メソッドには追加で日本語コメントを挿入しています。
"""
...
# 非同期解析結果取得の最大待機秒数 (poll_result のデフォルト値)
POLL_TIMEOUT_SECONDS = 180


@dataclass
class ReferenceDocItem:
    # 参照ドキュメント (Knowledge Source) 関連のローカルファイル情報を保持するデータ構造
    #  - filename: 元ファイル名
    #  - file_path: 元ファイルの絶対/相対パス
    #  - result_file_name: 解析 (OCR) 結果 JSON ファイル名 (ファイル名 + .result.json)
    #  - result_file_path: その結果 JSON のパス (skip_analyze=True 時などに利用)
    filename: str
    file_path: str
...
class AzureContentUnderstandingClient:
    """Content Understanding API を操作するための高レベルクライアント。

    主な公開操作:
        - get_all_analyzers / get_analyzer_detail_by_id
        - begin_create_analyzer / delete_analyzer / begin_analyze
        - get_prebuilt_document_analyze_result (prebuilt 利用のショートカット)
        - begin_create_classifier / begin_classify
        - poll_result (非同期操作結果のポーリング)
        - generate_training_data_on_blob / generate_knowledge_base_on_blob

    内部ユーティリティ:
        - _get_*_url 系: エンドポイント URL を生成
        - _get_training_data_config / _get_pro_mode_reference_docs_config: 要求ボディ断片生成
        - is_supported_doc_type_*: サポート拡張子検証
        - _upload_*: BLOB への非同期アップロード
    """

    # プリビルト アナライザー ID (サービス側で用意されている一般ドキュメント解析)
    ...
    # Knowledge Source 一覧 (sources.jsonl) のファイル名 (RAG/検索向け index 化に使う想定)
    ...
    # ドキュメント + テキスト系拡張子 (prebuilt ドキュメント解析でサポート) ※ MS Docs のリスト準拠
    ...
    # トレーニング / Pro モードでのみサポートされる純粋な「ドキュメント」拡張子 (画像+PDF 系)
    ...
    def __init__(
        ...
        # 認証: Subscription Key か token_provider(Bearer 取得コール可能) のどちらか必須
        # endpoint / api_version も必須
    ...
    def get_prebuilt_document_analyze_result(self, file_location: str) -> Dict[str, Any]:
        # プリビルト ドキュメント アナライザーを使った解析ショートカット
        # 返却値は poll_result により "succeeded" まで同期的に待機した JSON 結果
    ...
    async def upload_jsonl_to_blob(
        ...
        # JSON Lines 形式 (sources.jsonl など) を生成してアップロード
    ...
    async def generate_training_data_on_blob(
        ...
        """ラベル付きトレーニングデータ一式を BLOB にアップロードするヘルパー。

        フォルダ内の各ドキュメント (PDF/画像) に対して以下 2 ファイルの存在を検証:
          - <filename>.labels.json  (ラベル情報)
          - <filename>.result.json  (OCR / 解析結果 JSON)
        すべて揃っている場合のみ 3つ (元ファイル + labels + result) をアップロード。
        欠損があれば例外を投げて処理を中断します。
        """
